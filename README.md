...
野区行为分为探测、采样、战斗、宝藏、空中分支、地面运输与贸易六类。探测以望远镜读数为主，指南针与追溯指针承担辅助定位；采样以刷子、可疑沙、可疑沙砾与指定实体掉落承载；空中分支由带挽具的乐魂承载，骑手命中与拴绳吊运分别计入空中收益。所有动作类的津贴与操作分按当日月相权重折算，探测与空中分支对月相的弹性更大，新月日仍保留小额回报。每个动作即时入账小额行动津贴，并按当日月相强弱与近期热度折算为操作分。宝藏类涵盖野外容器开启与系统刷出的稀有宝箱开启两种事件，向图书管理员或制图师提交样本推进社区研究进度。
...
装备掉落在试炼宝库与配置标记的箱子、木桶、装饰罐开启时即时计算。直出概率由产出预估指数、当日月相 `phase_weight` 与 scope×模板 archetype 匹配系数 `af` 共同决定。
