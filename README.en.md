...
The wilderness is the high-risk geographic space defined by IMYVMWorldGeo. Players use Community zones as their home base and venture out into the wilderness. The wilderness stays open at all times; output and risk shift with the day's moon phase, with full and near-full moon as high-yield, high-risk windows and new moon as the quiet period.
...
At the community level, the Community treasury underwrites player policies, subscribes to wilderness index shares, and funds research progress. The shares market runs weekly: Monday opens issuance and orders, Saturday locks the price, Sunday pays out against the realized index. Once research crosses a tier threshold, the entire Community receives a cost discount and yield bonus on wilderness operations, alongside a one-time cash and equipment-pack reward.

Weekly settlement triggers every Sunday evening. The week's adventure gains are aggregated by action class and credited together with the index realization into player wallets; death-insurance circuit breakers reconcile first, then the index-share market settles. Settlement also emits the week's archive and a macro weekly report.
...
Per-player weekly equipment-output value uses two soft caps. Equipment drops and research-center crafts each have an independent budget, accumulated through the `item-basket` valuation; once equipment drops exceed their budget, direct-drop probability decays, and once research-center craft exceeds its budget, output efficiency decays instead of rejecting orders. Drops and craft still share the scope heat signal, keeping luck-driven and planning-driven players on one output rhythm.
...
Community research runs a single-axis accumulator. Community treasury funding and player sample submission through librarian or cartographer trades share the same progress counter:
...
Research carries two feedback cadences. When a player submits a qualified sample, an instant micro-allowance posts immediately: `bounty = clip(sample_value · k_bounty, 50, 1500) · (1 + yield_bonus[tier])`, default `k_bounty = 0.30`. When the community's `research_progress` crosses the next threshold, a one-shot upgrade reward fires at Sunday 18:00: a cash bonus `cash_bonus = α · tier · A_community^β` (default `α = 5000, β = 0.60`) enters the treasury, and an equipment bundle from `loot-windows.json` `tier_unlock_bundle.tier{k}` lands in the community shared storage. Tier 3 and above add milestone bundles to offset the longer investment cycle. When multiple thresholds cross in the same week, each tier settles independently in ascending order.
...
Sunday 18:00 starts the settlement sequence: session freeze, community development snapshot, scope index final values, death-insurance circuit-breaker reconciliation with prorate clawback, index-share market settlement, player and community `Cap_week` computation, macro feedback controller, operation-score conversion posting, research milestones, tier upgrades, next-cycle insurance renewal, weekly log archival, and cycle rollover. Each stage runs in a transaction and rolls back to the stage-start snapshot on failure.
...
The administrator-hot-reloadable configuration surface has two faces. Parameter weights live in TOML files covering economic parameters, index weights, research thresholds, insurance parameters, and settlement parameters. Output content lives in JSON files covering item-basket conversion coefficients, sample whitelists, container direct-drop and puzzle configurations, reading-tool tiers, and scope effect templates. Mathematical formulas, treated as a non-hot-reloadable code-layer concern, are concentrated inside the Index Engine, Weekly Settlement, and Macro Feedback Controller modules.
